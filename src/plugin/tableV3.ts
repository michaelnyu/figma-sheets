...
function selectColumn() {
  ...
  if (
    figma.currentPage.selection[0].parent?.parent.layoutMode === 'HORIZONTAL'
  ) {
    selectAdjacentCells();
  }
}
...
function restoreComponent(component) {
  var component: any = figma.getNodeById(
        ...
        if (findComponentById(figma.root.getPluginData('cellComponentID'))) {
          ...
          if (msg.columnCount < 51 && msg.rowCount < 51) {
            ...
            if (table) {
              ...
              if (figma.root.getPluginData('remember')) {
                message.remember =
                  figma.root.getPluginData('remember') == 'true';
              }
              ...
              if (figma.root.getPluginData('cellAlignment')) {
                message.cellAlignment = figma.root.getPluginData(
                  'cellAlignment',
                );
              }
